...
with tabs[1]:
    ...
    with col2:
        search_date = st.text_input("Search by Date", value=None)
    if st.button("Search"):
        st.session_state['search_results'] = service.search_documents(tag=search_tag if search_tag else None, date=str(search_date) if search_date else None)
    ...
    if st.session_state.reader_mode and st.session_state.selected_doc:
            ...
            image_dir = f"storage/pdf/{folder_name}"
            ...
            if not os.path.exists(image_dir):
                ...
                with col1:
                    if st.button('Prev') and current_page>0:
