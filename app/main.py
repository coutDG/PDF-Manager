...
with tabs[1]:
    ...
    if st.session_state.reader_mode and st.session_state.selected_doc:
        ...
        with container:
            st.write(
                "Files:",
                os.listdir(image_dir) if os.path.exists(image_dir) else "Not found",
            )
        ...
        if not os.path.exists(image_dir):
            ...
            with col1:
                if st.button("Prev") and current_page > 0:
                    analytics.record_app_visit("prev_click")
...
with tabs[2]:
    ...
    if df.empty:
        ...
        st.bar_chart(df, y= 'count', x = 'event')

    st.subheader("Document Progress")

    docs = service.get_all_document()
